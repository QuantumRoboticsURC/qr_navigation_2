...
class Follow_GPS(Node):
	...
	def followGPS(self,target_coordinates):
		
		if(self.state==0): #Checks if the state is the one assigned to FGPS
			state = Int8()
			arrived = Bool()
			self.get_logger().info("Entered Follow GPS v6")
			if(target_coordinates[0]!=None and target_coordinates[1]!=None): #Checks that the target coordinates are not null
				if(self.gps_coordinates[0]!=0.0 and self.gps_coordinates[1]!=0.0): #Checks that the gps readings are valid
					...
					self.x_target,self.y_target = ll2xy(target_coordinates[0],target_coordinates[1],self.orglat,self.orglong)
					#calculates the distance between the current position and the target position
					distance = distanceBetweenCoords(self.gps_coordinates[0],self.gps_coordinates[1],target_coordinates[0],target_coordinates[1])
