...
class Searching(Node):
	...
	def __init__(self):
		...
		#Default archimedean spiral graph
		#self.scale = 7
		#self.turns = 7
		#self.points_distance = 150
		#self.continuous_points_number = 500
		#Basic archimedean spiral graph
		self.scale = 10
		self.turns = 3
		self.points_distance = 350
		self.continuous_points_number = 500
		self.navigation_point_uncertainty = 10
		self.set_default_configuration()
		self.sub_state = self.create_subscription(Int8, '/state', self.state_callback, 1)
		...
		self.timer1 = self.create_timer(0.001,self.followGPS2,callback_group=timer_group)
	...
	def timer_callback(self):
		...
		print("Next: ", self.target_navigation_coordinates)
		print("Distance: ", self.remaining_distance)
	...
	def calc_angle(self):
		dX,dY = ll2xy(self.target_coordinates[0],self.target_coordinates[1],self.orglat,self.orglong)
		...
		target_angle = ((math.atan2(dY-self.y_rover,dX-self.x_rover))+2*math.pi)%(2*math.pi)
		#print(self.gps_coordinates)
		#print(self.target_coordinates)
		#print(f"dx,dy, dT {dX,dY,target_angle}")
		#print(f"Rover coords x,y,a {self.x_rover,self.y_rover,self.angle}")
		self.dX = dX
		self.dY = dY
		return target_angle
	...
	def angle_correction(self,target_angle):
		...
		print(self.x_rover,self.y_rover)
